const refreshChannel = typeof BroadcastChannel !== 'undefined' ? new BroadcastChannel('portfolio-refresh') : null;

document.addEventListener('DOMContentLoaded', async () => {
    document.getElementById('current-year').textContent = new Date().getFullYear();

    window.addEventListener('portfolio:refresh', async () => {
        await loadPublicData();
    });

    window.addEventListener('storage', (event) => {
        if (event.key === 'portfolio:refresh' || event.key === 'portfolio:refresh:admin') {
            loadPublicData();
        }
    });
    ...
    if (refreshChannel) {
        refreshChannel.onmessage = (event) => {
            if (event.data?.type === 'portfolio-refresh') {
                loadPublicData();
            }
        };
    }
...
async function loadPublicData() {
    try {
        // Menggunakan endpoint /api/main-profile dengan cache busting agar data selalu terbaru
        const response = await fetch(`/api/main-profile?t=${Date.now()}`);
